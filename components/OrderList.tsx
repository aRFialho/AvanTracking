...
export const OrderList: React.FC<OrderListProps> = ({ orders, onFetchSingle }) => {
  ...
  const validOrders = useMemo(() => orders.filter(o => o.status !== OrderStatus.CANCELED), [orders]);

  const carriers = useMemo(() => Array.from(new Set(validOrders.map(o => o.freightType))).sort(), [validOrders]);
  const marketplaces = useMemo(() => Array.from(new Set(validOrders.map(o => o.salesChannel))).sort(), [validOrders]);
  ...
  const filteredOrders = useMemo(() => {
    return validOrders.filter(o => {
      ...
      const matchCarrier = carrierFilter === 'ALL' || o.freightType === carrierFilter;
      const matchMkt = marketplaceFilter === 'ALL' || o.salesChannel === marketplaceFilter;

      // 3. Date Range (Estimated Delivery)
      let matchDate = true;
      if (dateRangeStart) {
          matchDate = matchDate && new Date(o.estimatedDeliveryDate) >= new Date(dateRangeStart);
      }
      if (dateRangeEnd) {
          matchDate = matchDate && new Date(o.estimatedDeliveryDate) <= new Date(dateRangeEnd);
      }

      return matchText && matchStatus && matchCarrier && matchMkt && matchDate;
    });
  }, [validOrders, searchText, statusFilter, carrierFilter, marketplaceFilter, dateRangeStart, dateRangeEnd]);
  ...
  // Modern Status Badge
  const StatusBadge = ({ status, delayed }: { status: OrderStatus, delayed: boolean }) => {
    const baseClass = "inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider border";
    
    if (delayed && status !== OrderStatus.DELIVERED && status !== OrderStatus.CHANNEL_LOGISTICS) {
      return <span className={clsx(baseClass, "bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-900/30")}>Atrasado</span>;
    }
    
    switch (status) {
      case OrderStatus.DELIVERED:
        return <span className={clsx(baseClass, "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-900/30")}>Entregue</span>;
      case OrderStatus.SHIPPED:
        return <span className={clsx(baseClass, "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-900/30")}>Trânsito</span>;
      case OrderStatus.DELIVERY_ATTEMPT:
        return <span className={clsx(baseClass, "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-900/30")}>Rota</span>;
      case OrderStatus.FAILURE:
      case OrderStatus.RETURNED:
      case OrderStatus.CANCELED:
        return <span className={clsx(baseClass, "bg-slate-100 text-slate-700 border-slate-200 dark:bg-white/10 dark:text-slate-300 dark:border-white/5")}>Falha</span>;
      case OrderStatus.CHANNEL_LOGISTICS:
        return <span className={clsx(baseClass, "bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/20 dark:text-purple-400 dark:border-purple-900/30")}>Logística do Canal</span>;
      default:
        return <span className={clsx(baseClass, "bg-gray-50 text-gray-600 border-gray-200 dark:bg-white/5 dark:text-gray-400 dark:border-white/5")}>Pendente</span>;
    }
  };
          ...
          {showFilters && (
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-in slide-in-from-top-2 duration-200 bg-white dark:bg-dark-card">
                ...
                <div className="col-span-1 lg:col-span-4 flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-white/5">
                    <button onClick={clearFilters} className="text-slate-500 text-sm hover:text-slate-800 dark:hover:text-white px-4 py-2 font-medium">
                        Limpar Filtros
                    </button>
                    <button 
        ...
        <div className="absolute inset-0 overflow-auto">
          <table className="w-full text-sm text-left border-collapse">
            <thead className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase bg-slate-50 dark:bg-dark-card sticky top-0 z-10 shadow-sm backdrop-blur-md">
              <tr>
                <th className="px-4 py-3 whitespace-nowrap bg-slate-50 dark:bg-[#11131f]">ID / Pedido</th>
                <th className="px-4 py-3 whitespace-nowrap bg-slate-50 dark:bg-[#11131f]">Emissão</th>
                <th className="px-4 py-3 whitespace-nowrap bg-slate-50 dark:bg-[#11131f]">Marketplace</th>
                <th className="px-4 py-3 whitespace-nowrap bg-slate-50 dark:bg-[#11131f]">Transportadora</th>
                <th className="px-4 py-3 whitespace-nowrap bg-slate-50 dark:bg-[#11131f]">Prev. Entrega</th>
                <th className="px-4 py-3 whitespace-nowrap bg-slate-50 dark:bg-[#11131f]">Última Movimentação</th>
                <th className="px-4 py-3 whitespace-nowrap text-center bg-slate-50 dark:bg-[#11131f]">Status</th>
                <th className="px-4 py-3 whitespace-nowrap text-right bg-slate-50 dark:bg-[#11131f]">Ação</th>
              </tr>
            </thead>
                ...
                <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">
                    <div className="flex flex-col">
                        <span className="bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded text-xs w-fit mb-0.5">#{order.orderNumber}</span>
                        <span className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[150px]">{order.customerName}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-300 whitespace-nowrap">
                    <div className="flex flex-col">
                        <span className="text-xs">{order.lastUpdate ? new Date(order.lastUpdate).toLocaleDateString() : '-'}</span>
                        <span className="text-[10px] text-slate-400">{order.lastUpdate ? new Date(order.lastUpdate).toLocaleTimeString().slice(0,5) : ''}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <StatusBadge status={order.status} delayed={order.isDelayed} />
                  </td>
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                           <Globe className="w-5 h-5 text-accent dark:text-neon-blue"/> Busca Externa
                        </h3>
                        <p className="text-xs text-slate-500">Consultar Intelipost via API</p>
                    </div>
                    <button onClick={() => setIsSearchModalOpen(false)} className="text-slate-500 hover:text-white"><X className="w-5 h-5"/></button>
                </div>
                
                <form onSubmit={executeApiSearch} className="space-y-4">
                    <div>
                        <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Número do Pedido</label>
                        <input 
                            type="text" 
                            required
                            autoFocus
                    ...
                    <div className="pt-2 flex gap-3">
                        ...
                        >
                            {isFetchingSingle ? <Loader2 className="w-5 h-5 animate-spin"/> : "Consultar"}
                        </button>
                    </div>
                </form>
            </div>
