...
import { AlertTriangle, Clock, CalendarX, Eye, AlertOctagon, CheckCircle } from 'lucide-react';
...
export const AlertsView: React.FC<AlertsViewProps> = ({ orders }) => {
  ...
  const riskyOrders = useMemo(() => {
    return orders.filter(o => {
        ...
        const hasFailure = o.status === OrderStatus.FAILURE || o.status === OrderStatus.RETURNED;
    ...
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 p-6 rounded-xl flex items-center justify-between">
              <div>
                  <p className="text-red-600 dark:text-red-400 font-medium text-sm">Total de Alertas</p>
                  <h3 className="text-3xl font-bold text-red-800 dark:text-white">{riskyOrders.length}</h3>
              </div>
          ...
          <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-900/30 p-6 rounded-xl flex items-center justify-between">
              <div>
                  <p className="text-orange-600 dark:text-orange-400 font-medium text-sm">Atraso Crítico (+5 dias)</p>
          ...
          <div className="bg-white dark:bg-dark-card border border-slate-200 dark:border-white/10 p-6 rounded-xl flex items-center gap-4">
              <div className="flex-1">
                 <label className="text-sm font-medium text-slate-600 dark:text-slate-300 block mb-2">Filtrar por dias de atraso</label>
                 <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="w-8 text-sm font-bold text-slate-700 dark:text-white">{minDaysDelayed}+</span>
                 </div>
              </div>
          </div>
      </div>
        ...
        <div className="absolute inset-0 overflow-auto">
            {riskyOrders.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
                    ...
                    <p className="font-medium text-lg text-slate-600 dark:text-slate-300">Tudo certo!</p>
                ...
                <div className="divide-y divide-slate-100 dark:divide-white/5">
                    {riskyOrders.map(order => {
                        ...
                        return (
                            <div key={order.id} className="p-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <h4 className="font-bold text-slate-800 dark:text-white">Pedido #{order.orderNumber}</h4>
                                        ...
                                        <p className="text-slate-600 dark:text-slate-400 text-sm mt-0.5">{order.customerName}</p>
                                        <div className="flex items-center gap-3 mt-1 text-xs text-slate-500 dark:text-slate-500">
                                            <span className="flex items-center gap-1">
                                                <CalendarX className="w-3 h-3" /> Previsto: {new Date(order.estimatedDeliveryDate).toLocaleDateString()}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                                    <div className="text-right">
                                        <p className="text-xs text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">Tempo de Atraso</p>
                                        <p className="text-xl font-bold text-red-600 dark:text-red-400">{days} dias</p>
                                    </div>
                                    <button 
