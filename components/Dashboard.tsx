...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const uniqueCarriers = useMemo(
    () =>
      Array.from(
        new Set(orders.map((o) => normalizeCarrierName(o.freightType))),
      ).sort(),
    [orders],
  );
  const uniqueMarketplaces = useMemo(
    () => Array.from(new Set(orders.map((o) => o.salesChannel))).sort(),
    [orders],
  );
  ...
  const stats = useMemo(() => {
    ...
    const onRoute = filteredOrders.filter(
      (o) => o.status === OrderStatus.DELIVERY_ATTEMPT,
    ).length;
    ...
    const deliveryFailures = filteredOrders.filter(
      (o) =>
        o.trackingHistory &&
  ...
  const monthSummary = useMemo(() => {
    ...
    const currOnTime = currentMonthOrders.filter(
      (o) => o.status === OrderStatus.DELIVERED && !o.isDelayed,
    ).length;
    const prevOnTime = prevMonthOrders.filter(
      (o) => o.status === OrderStatus.DELIVERED && !o.isDelayed,
    ).length;
    ...
    return {
      ...
      onTimePct:
        currentMonthOrders.length > 0
          ? ((currOnTime / currentMonthOrders.length) * 100).toFixed(0)
          : 0,
      onTimeGrowth,
      activeDelayed,
    };
